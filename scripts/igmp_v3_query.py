...
# Send an IGMPv3 general or group-specific query.
# TODO: Get source-specific queries working.
...
def main():
    ...
    parser.add_option("-S", "--igmp_sources",
                      ...
                      help="Comma-delimited list of IPv4 sources for a group-and-source specific query.")
    ...
    igmp = igmpv3_query(igmp_sources)
    igmp.maxresp = int(options.maxresp) * 10       # in units of deciseconds
    igmp.qrv = 2		    # SHOULD NOT be 1, MUST NOT be 0
    igmp.qqic = 10		    # I query every 10 seconds

    if options.igmp_group is None:
        # General query.
        ip.dst = inet_atol("224.0.0.1")
        igmp.group = 0
    else:
